...
def main():
    ...
    try:
        ...
        print(new_login)
        test_user = create_user(session_local, new_login,
                                '1234', role=UserRole.USER, balance=100)
        print(test_user.id, test_user.login,
              test_user.role, test_user.password_hash)
        ...
        print(
            f'транзакция: amount: {dep_t.amount}, created_at: {dep_t.created_at}, task: {dep_t.task_id}, type: {dep_t.transaction_type}')

        model = session_local.query(MLModel).filter_by(
            model_id="resnet18").first()
        if model is None:
            raise ValueError("Не найдена демо-модель resnet18")

        print("создаем ml задачу")
        task_test = create_task(
            session_local, test_user.id, model.id, "test_data.png")
        print(
            f"task: created_at: {task_test.created_at}, data: {task_test.data}, status: {task_test.status}")

        print("создаем транзакцию списания средств")
        credit_t = withdraw_balance(
            session_local, test_user.id, 50, task_test.id)
        print(
            f'транзакция: amount: {credit_t.amount}, created_at: {credit_t.created_at}, task: {credit_t.task_id}, type: {credit_t.transaction_type}')

        user_after_transactions = get_user_by_id(session_local, test_user.id)
        print("проверяем пользователя после транзакций")
        print(
            f"user: login: {user_after_transactions.login}, balance: {user_after_transactions.balance}, tr_his: {user_after_transactions.transactions}")
        print(f"tasks_his: {user_after_transactions.tasks}")

        print("проверяем историю транзакций пользователя:")
        transactions = get_user_transactions_history(
            session_local, test_user.id)
